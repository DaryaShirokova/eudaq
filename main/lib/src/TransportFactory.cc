...
namespace eudaq {
    ...
    static map_t &TransportMap() {
      ...
      if (!initialised) {
        ...
        TransportFactory::Register(
            MakeTransportInfo<NULLServer, NULLClient>(NULLServer::name));
        TransportFactory::Register(
            MakeTransportInfo<TCPServer, TCPClient>(TCPServer::name));
      }
      static map_t m;
      return m;
    }
  ...
  void TransportFactory::Register(const TransportInfo &info) {
    // std::cout << "DEBUG: TransportFactory::Register " << info.name <<
    // std::endl;
